#include <chrono>
#include "BoolSignalGenerator.h"

ADTF_FILTER_PLUGIN(FILTER_NAME, OID, BoolSignalGenerator)

BoolSignalGenerator::BoolSignalGenerator(const tChar *__info) : cTimeTriggeredFilter(__info)
{
    ...
    SetPropertyBool(SIGNAL_PROPERTY, SIGNAL_DEFAULT);
...
BoolSignalGenerator::~BoolSignalGenerator()
= default;

tResult BoolSignalGenerator::Init(tInitStage eStage, __exception)
{
...
tResult BoolSignalGenerator::Shutdown(tInitStage eStage, __exception)
{
    return cFilter::Shutdown(eStage, __exception_ptr);
}


tResult BoolSignalGenerator::CreateDescriptions(IException **__exception_ptr)
{
    ...
    //get SignalValue
    tChar const *signalValueDescription = descManager->GetMediaDescription("tBoolSignalValue");
    RETURN_IF_POINTER_NULL(signalValueDescription);
    signalValueStruct = new cMediaType(0, 0, 0, "tBoolSignalValue", signalValueDescription,
...
tResult BoolSignalGenerator::CreateOutputPins(IException **__exception_ptr) {
...
tResult BoolSignalGenerator::Cycle(IException **__exception_ptr) {

    const auto value = GetPropertyBool(SIGNAL_PROPERTY);
    TransmitValue(signalPin, value);

    RETURN_NOERROR;
}

tResult BoolSignalGenerator::TransmitValue(cOutputPin &pin, tBool value)
{
    if (!pin.IsConnected())
    {
        RETURN_NOERROR;
